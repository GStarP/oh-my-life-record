...
本地存储统一用 IndexedDB：`records`（按 `time` 索引，游标分页实现懒加载）、`images`（仅待上传图片的 Blob，附 createdAt 供孤儿兜底清理，上传成功即删）、`partitionState`（按月同步状态）、`typeTemplates`（类型 → 当前模板）和 `typeTemplateState`（全局模板同步状态）。`StorageAdapter.getImageBlob` 隐藏暂存项元数据。数据库初始且当前版本固定为 `1`，直接创建全部对象存储，不提供旧结构升级或兼容读取；正式投入使用前若结构重置，直接清理开发期本地与云端数据。
